...
class OIDCController extends AbstractController
{
    ...
    /**
     * @Route("/_oidc/login", name="ewop_oidc_login")
     */
    ...
    /**
     * @Route("/_oidc/callback", name="ewop_oidc_callback")
     */
    public function callback(Request $request)
    {
        ...
        // Otherwise, append a '?'
        $app_uri = strpos($app_uri, '?') === false ? $app_uri.'?' : $app_uri.'&';
        ...
        if ($response->getStatusCode() >= 400) {
            $this->logger->error('Failed to fetch token: '. $response->getContent());
    ...
    /**
     * @Route("/_oidc/status", name="ewop_oidc_status")
     */
    public function status(Request $request)
    {
        ...
        $accessTokenExpiresAt = $request->getSession()->get('ewop_token_expires_at');
        $remainingSeconds = max(0, $accessTokenExpiresAt - time());

        // remaining seconds until refresh token expires
        $refreshTokenExpiresAt = $request->getSession()->get('ewop_refresh_token_expires_at');
    ...
    /**
     * @Route("/_oidcp/{api}/{path}", name="ewop_oidc_proxy", requirements={"api"="[a-z]+","path"=".+"})
     */
    ...
    private function checkAndRefreshToken($session): bool
    {
        ...
        if ($accessTokenExpiresAt && time() > $accessTokenExpiresAt - 5) {
            ...
            $refreshTokenExpiresAt = $session->get('ewop_refresh_token_expires_at');
    ...
    private function tokenResponseToSession($session, $data)
    {
        ...
        $session->set('ewop_refresh_token_expires_at', time() + $refreshTokenExpiresIn);
